...
from transformers import get_linear_schedule_with_warmup, get_cosine_schedule_with_warmup, AutoTokenizer, AutoModel
...
def parse_args():
    ...
    parser.add_argument("--early_stopping_threshold", type=float, default=0.0005, help="Minimum change to qualify as an improvement.")
    parser.add_argument("--weight_decay", type=float, default=0.005, help="Weight decay to use.")
...
if __name__ == '__main__':
    ...
    logger.add(f'log/rec_v1/lr_{args.learning_rate}/{local_time}.log', level='DEBUG' if accelerator.is_local_main_process else 'ERROR')
    ...
    tokenizer = AutoTokenizer.from_pretrained(args.tokenizer, model_max_length=512, truncate = True)  # Adjust 512 as needed
    ...
    text_tokenizer = AutoTokenizer.from_pretrained(args.text_tokenizer, model_max_length=512, truncate = True)  # Adjust 512 as needed
    ...
    data_collator = CRSRecDataCollator(
        ...
        context_max_length=min(args.context_max_length, tokenizer.model_max_length),
        entity_max_length=min(args.entity_max_length, tokenizer.model_max_length),
        pad_entity_id=kg['pad_entity_id'],
        prompt_tokenizer=text_tokenizer, 
        prompt_max_length=min(args.prompt_max_length, text_tokenizer.model_max_length),
    )
        ...
        for batch in tqdm(valid_dataloader):
            with torch.no_grad():
                ...
                try:
                    ...
                    # ranks = sorted_indices[:, :50].tolist()
                except RuntimeError as e:
                    print(f"Error occurred. Logits shape: {logits.shape}")
